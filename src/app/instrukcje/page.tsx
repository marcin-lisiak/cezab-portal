...
export default async function InstrukcjePage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const sp = await searchParams;
  const q = (sp?.q || "").trim();
  const assets = await prisma.asset.findMany({
    where: { type: "INSTRUCTION", ...(q ? { title: { contains: q } } : {}) },
  ...
  return (
    <main className="mx-auto max-w-5xl p-6">
      <h1 className="text-2xl font-semibold mb-4">Instrukcje</h1>
      <form className="mb-4">
